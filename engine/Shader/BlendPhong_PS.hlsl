...
float4 main(float3 _WorldPos : Position, float3 _Norm : Normal, float3 _Color : Color) : SV_TARGET
{
    ...
    return float4(saturate((diff + Ambient + CalculateSpecularIntensity(vtol, att, _WorldPos, _Norm)) * _Color), 1.0f);
}
