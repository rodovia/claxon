/* PhongNormalMapOS_PS.hlsl -- Phong shader with object space normal map */
...
float4 main(float3 _WorldPos : Position, float3 _Norm : Normal, 
        float3 _Ignored1 : Tangent, float3 _Ignored2 : Bitangent, 
        float4 _Ignored3 : SV_Position, float2 _TexCoord : Texcoord) : SV_TARGET
{
    if (EnableNormalMap)
    {
    ...
    else
    {
        ...
        _Norm.x = normalSample.x * 2.0f - 1.0f;
        _Norm.y = -normalSample.y * 2.0f + 1.0f;
        _Norm.z = -normalSample.z * 2.0f + 1.0f;
        _Norm = mul(_Norm, (float3x3) ModelView);
    }
